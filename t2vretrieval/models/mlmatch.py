...
class RoleGraphMatchModelConfig(t2vretrieval.models.globalmatch.GlobalMatchModelConfig):
  def __init__(self):
    ...
    ## 这个地方设定config设定的属性都会被model.json的覆盖掉，因为有个load和load_from_dict
...
class RoleGraphMatchModel(t2vretrieval.models.globalmatch.GlobalMatchModel):
  ...
  def forward_video_embed(self, batch_data):
    ...
    vid_sent_embeds, vid_verb_embeds, vid_noun_embeds, logits, max_len = self.submods[VISENC](vid_fts, vid_lens)
    return {
  ...
  def forward_text_embed(self, batch_data):
    ...
    sent_embeds, verb_embeds, noun_embeds, sent_logits = self.submods[TXTENC](
  ...
  def generate_scores(self, **kwargs):
    ...
    return sent_scores, verb_scores, noun_scores, concept_verb_scores, concept_noun_scores
  ...
  def forward_loss(self, batch_data, step=None):
    ...
    sent_scores, verb_scores, noun_scores, concept_verb_scores, concept_noun_scores = self.generate_scores(**enc_outs)
    ...
    concept_loss = self.criterion(scores2)
    #  may be use focal loss?
    # vbce_loss = -torch.mean(torch.sum(verb_label * torch.log(enc_outs['logits'][0]), dim=1), dim=0)
    # nbce_loss = -torch.mean(torch.sum(noun_label * torch.log(enc_outs['logits'][1]), dim=1), dim=0)
    ...
    vbce_loss = 0.1*torch.mean(vbce_loss/v_mask_sum)
    nbce_loss = torch.sum(noun_concept_mask*self.criterion_bce(enc_outs['logits'][1], noun_concept_label), dim=1)
    nbce_loss = 0.1*torch.mean(nbce_loss/n_mask_sum)
    vbce_sent_loss = torch.sum(verb_concept_mask*self.criterion_bce(enc_outs['sent_logits'][0], verb_concept_label), dim=1)
    vbce_sent_loss = 0.1*torch.mean(vbce_sent_loss/v_mask_sum)
    nbce_sent_loss = torch.sum(noun_concept_mask*self.criterion_bce(enc_outs['sent_logits'][1], noun_concept_label), dim=1)
    nbce_sent_loss = 0.1*torch.mean(nbce_sent_loss/n_mask_sum)
    # vbce_loss = 0.5*torch.mean(torch.mean(verb_concept_mask*self.criterion_bce(enc_outs['logits'][0], verb_concept_label), dim=1))
    # nbce_loss = 0.5*torch.mean(torch.mean(noun_concept_mask*self.criterion_bce(enc_outs['logits'][1], noun_concept_label), dim=1))
    # vbce_sent_loss = 0.5*torch.mean(torch.mean(self.criterion_bce(enc_outs['sent_logits'][0], verb_concept_label), dim=1))
    # nbce_sent_loss = 0.5*torch.mean(torch.mean(self.criterion_bce(enc_outs['sent_logits'][1], noun_concept_label), dim=1))
    # print('SSSSSSSSSSSSSSSSSSSSSSSSSSSSS')
    # print(enc_outs['logits'][0][0][:20])
    # print(enc_outs['sent_logits'][0][0][:20])
    # print(verb_concept_label[0][:20])
    # print('EEEEEEEEEEEEEEEEEEEEEEEEEEEEE')
    #############################
    fusion_loss = self.criterion(scores) 

    if self.config.loss_weights is None:
      loss = fusion_loss + (vbce_loss+nbce_loss) + (vbce_sent_loss+nbce_sent_loss) + concept_loss
    else:
    ...
    if step is not None and self.config.monitor_iter > 0 and step % self.config.monitor_iter == 0:
      neg_scores = scores2.masked_fill(torch.eye(len(scores2), dtype=torch.bool).to(self.device), -1e10)
      self.print_fn('\tstep %d: pos mean scores %.2f, hard neg mean scores i2t %.2f, t2i %.2f'%(
        step, torch.mean(torch.diag(scores2)), torch.mean(torch.max(neg_scores, 1)[0]), 
  ...
  def evaluate_scores(self, tst_reader):
    ...
    K = K +2
    assert K == 5, 'Note that this error indicates losing two concept space scores!'
  ...
  def evaluate(self, tst_reader, return_outs=False):
    ...
    fused_scores = np.mean(scores[3:], 0)
